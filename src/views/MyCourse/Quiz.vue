<template>
  <div>
    ...
    <div class="flex justify-center h-screen"
         :class="darkMode?`bg-youtube text-gray-300 p-5 ${isExam?``:`items-center`}`:`bg-white m-5 shadow ${isExam?``:`items-center`}`">
      <!-- Note -->
      <div class="rounded-2xl px-10 py-5"
           :class="darkMode?`bg-secondary text-gray-300 shadow`:`bg-white shadow-md border-t border-gray-100`"
           v-if="!isExam">
        ...
        <div class="text-sm mt-10">
          <div v-for="(quizzes, index) in certificatesQuiz.list" :key="index" :class="order != index?`hidden`:``">
            <div :class="darkMode?`bg-secondary`:`bg-gray-50`"
                 class="shadow rounded-md py-5 px-10 rgba flex-col overflow-y-scroll my-scroll max-w-xl">
                  ...
                  <label class="flex items-center relative">
                    <div class="relative">
                      ...
                      <div class="h-5 w-5 rounded border mr-5 relative"
                           :class="darkMode?`border-`:`border-gray-400`"></div>
                    </div>
                    <div>
          ...
          <div :class="isPrevious?``:`invisible`">
            <button class="focus:outline-none rounded px-5 py-1 flex items-center"
              ...
              <div class="transform rotate-90 relative -left-1">
                <ChevronIcon fill="#fff" :size="18"></ChevronIcon>
              </div>
              <div>{{ $t('2126') }}</div>
            </button>
          </div>
    ...
    <ViewExam :quiz="certificatesQuiz" :result="result" v-if="showResult" :selectedQuiz="selectedQuiz"
              @exit="exit"></ViewExam>
