<template>
  <div class="fixed inset-0 font-khmer_os w-full left-0 top-0 bg-black bg-opacity-90 flex justify-center items-center"
       style="z-index:70;">
    <div class="rounded-xl shadow-lg w-96 relative" :class="darkMode?`bg-secondary text-gray-300`:`bg-white`">
        ...
        <div class="text-13px overflow-y-scroll" style="max-height: 550px;">
          <div class="p-5">
            <div class="flex justify-between items-center">
              <input type="text" id="invoice_number" class="absolute" :value="receiptDetail.receipt"
                     style="z-index:-10">
              <div class="whitespace-nowrap">{{ $t('invoce_no') }}</div>
              <div class="flex-1 text-right" :class="darkMode?`text-gray-300`:`text-heart`">
                {{ receiptDetail.receipt }}
              </div>
              <div class="ml-2 cursor-pointer" @click="copyText()">
                <CopyIcon :size="20" :fill="darkMode?`#D1D5DB`:`#000000`"/>
              </div>
            </div>
            ...
            <div class="course mt-3">
              ...
              <ul>
                <li class="flex justify-between items-center my-2"
                  ...
                  <div>
                    <span class="font-mono">${{ parseFloat(receipt.amount) }}</span>
          ...
          <div class="p-5 mt-1">
            <div class="total flex justify-between items-center">
              <div>{{ $t('total') }}</div>
              <div class="text-heart font-mono font-bold">
                ${{ parseFloat(receiptDetail.amount) }}
              </div>
            </div>
            <div v-if="!success">
              <div class="mt-3 text-center text-13px">{{ $t('payment_method') }}</div>
              <div class="mt-6">
                <div v-for="(rc, key) in receiptDetail.payment_methods" :key="key">
                  <PaymentOption
                      @onPay="onPay(rc)"
                      :logo="rc.logo"
                      :name="rc.name"
            ...
            <template v-else>
              ...
              <div class="flex justify-center items-center text-sm font-semibold mt-3">
                វិក្កយបត្របានបង់ប្រាក់ហើយ
              </div>
            </template>
          </div>
        </div>
...
import PaymentOption from "@/views/MyCourse/components/PaymentOption";
...
export default {
  ...
  methods: {
    ...
    onPay(rc) {
      if (rc.option == 'qrcode') {
        this.$emit('onPay', this.receiptDetail)
        return
      }
      helper.errorMessage('0029')
    ...
    copyText() {
      let copyText = document.getElementById("invoice_number");
      copyText.select();
