    ...
    <div class="flex mt-3 px-5">
      ...
      <div class="w-3/5">
        <div :class="darkMode?`bg-youtube text-gray-300`:`bg-white pb-5 shadow rounded-b-lg text-sm`">
          <div class="relative" @mouseover="() => {this.fullScreenBtn = true}"
               @mouseleave="() =>{this.fullScreenBtn = false}">
            <div
                class="flex justify-center items-center bg-black bg-opacity-70 rounded absolute top-5 left-5 cursor-pointer z-50 h-8 w-8"
                v-if="fullScreenBtn" @click="toggleFullScreen">
            ...
            <video controlsList="nodownload" autoplay class="m-auto w-full outline-none" id="vid"
                   ref="eVideo"
            ...
            <div class="absolute w-full bottom-14 flex justify-end items-end px-10">
              <div
                  class="bg-black bg-opacity-80 w-full rounded-md flex items-center justify-between h-10 text-gray-300 mb-10 px-5">
                <!-- Play & pause -->
                <div v-if="showPlay" @click="playPause">
                  <PlayIcon fill="#ffffff"/>
                </div>
                <div v-else @click="playPause">
                  <PauseIcon fill="#ffffff"/>
                </div>
                <!-- Duration -->
                ...
                <div class="flex-1 relative">
                  ...
                  <div class="bg-white absolute rounded" style="height:4px;bottom:10px;z-index:1"
                       :style="{width:`${rangeSliderWidth}%`}"></div>
                </div>
                <!-- Duration -->
                <div class="px-5 opacity-80">
                ...
                <div class="mr-5 cursor-pointer relative">
                    ...
                    <div class="bg-black absolute bottom-8 w-48 right-0 bg-opacity-70 rounded-md pt-2 overflow-hidden"
                      ...
                      <div>
                        <div
                          ...
                          <span class="text-sm absolute left-5" v-if="defaultQuality ===1">&#10003;</span>{{
                            $t('auto')
                          }}
                        </div>
                        <div
                  ...
                  <div class="relative">
                    ...
                    <div @mouseover="() =>{this.showSound = true}" v-else>
                      <SoundIcon/>
                    </div>
                    <div
                        class="range-slider absolute w-14 h-12 flex items-center justify-center transform -rotate-90 -top-14 -left-5"
                        @mouseleave="() =>{this.showSound = false}"
                        @mouseover="() =>{this.showSound = true}">
                      <input type="range" min="0" max="100" step="1" v-model="defaultVolumeRange"
                             id="volumeSlider"
                             class="w-full cursor-pointer slider relative left-2"
                             @change="setVolume"
      ...
      <div class="w-2/5">
        <div class="flex-1 ml-5" :class="darkMode?`text-gray-300`:`text-black`">
          <div>
            <div :class="darkMode?`bg-secondary border-b border-button text-gray-300`:`bg-white text-primary`"
                 class="h-12 px-5 flex items-center font-bold text-sm">
              <span>{{ $t('more_video') }}</span>
            </div>
          </div>
          <div class="h-screen overflow-y-scroll pb-60 mt-3">
            <div v-for="(list, index) in details.list" :key="index" class="text-sm cursor-pointer"
                 @click="nextVideo(list, index)">
              <div class="flex rounded-md p-3"
                   :class="darkMode?`${order === index?`bg-white text-black`:`bg-secondary`} mb-3`:`${order  === index?`bg-gray-300`:`bg-white`} shadow-md mb-3`">
                <div class="relative w-2/5 mr-3 cursor-pointer relative">
                  <img :src="list.thumbnail" class="rounded-md">
                  <div class="absolute h-1 bg-red-600 bottom-0 left-0" v-if="list.last_watch"
                ...
                <div class="flex flex-col justify-between py-2 w-full flex-1">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <template v-if="!canWatch(list)">
                      <LockIcon :width="20" :height="20" :fill="darkMode?`#fff`:`#000`"/>
                    </template>
                  </div>
                </div>
              </div>
      ...
      <BuyMsg
          v-if="isPayment"
          @cancelModal="() => {this.isPayment = false}"
          @yes="yes"/>
      <Cart
          v-if="showCart" @closeCart="()=>{this.showCart = false}"
          @showInvoice="(data) =>{ this.showCart = false; this.showReceipt = true;this.receiptDetail = data}"/>
      <ReceiptInfo
          v-if="showReceipt"
...
export default {
  ...
  methods: {
    ...
    canWatch(videos) {
      return (this.details.is_buy == 1 || videos.free_watch == 1);
        ...
        if (document.getElementById("vid") !== null) {
          ...
          this.seekSlider.addEventListener(
              ...
              event => {
                ...
                this.seekSlider.style.background = `linear-gradient(90deg, rgb(255, 255, 255) ${
                    event.target.value
    ...
    timeUpdate() {
      ...
      this.seekSlider.style.background = `linear-gradient(90deg, rgb(255, 255, 255) ${nt}%, rgb(151, 151, 151) ${nt}%)`;
