<template>
  <div class="h-screen">
    ...
    <div :class="darkMode?`bg-dark text-gray-300 bg-cover`:`bg-light-mode bg-cover`"
         class="rounded-xl bg-repeat m-5 h-full flex flex-col">
      ...
      <div class="px-5 flex-1 h-screen">
        <div class="flex items-center justify-center w-full my-5">
          <div class="flex items-center rounded-xl w-40 justify-center h-13 mr-5 shadow-lg cursor-pointer"
               :class="darkMode?`bg-button`:`bg-primary text-white`" @click="openLink(messanger)">
            <div class="mr-3">
              <MessagerIcon/>
            </div>
            <div>
          ...
          <div class="flex items-center rounded-xl w-40 justify-center h-13 mr-5 shadow-lg cursor-pointer"
               :class="darkMode?`bg-button`:`bg-primary text-white`" @click="openLink(telegram)">
            <div class="mr-3">
              <TelegramIcon/>
            </div>
            <div>
          ...
          <div class="flex items-center rounded-xl w-40 justify-center h-13 mr-5 shadow-lg cursor-pointer"
               :class="darkMode?`bg-button`:`bg-primary text-white`" @click="()=>{$router.push('help')}">
            <div class="mr-3">
              <VideoIcon/>
            </div>
            <div>
        ...
        <div class="mt-10 h-full" style="position:relative;z-index:1">
          <div class="relative h-screen z-50 pt-3 justify-between flex flex-col">
            <ul class="overflow-y-scroll flex-1 px-5 pb-100" ref="feed" @scroll="getMoreMessage"
                style="z-index:999;">
              <div class="fixed bg-black w-full h-full left-0 top-0 bg-opacity-70 z-50" v-if="replyId"
                ...
                <template v-if="message.content.type === 1">
                  <div :class="message.is_admin?`flex justify-start`:`flex justify-end`"
                    ...
                    <div :class="darkMode?`text-gray-500`:`text-gray-700`"
                         class="text-xs whitespace-nowrap uppercase">
                  ...
                  <div :class="message.is_admin?`flex justify-start`:`flex justify-end`"
                    ...
                    <div>
                      <div v-if="message.reply !== undefined">
                        ...
                        <div class="relative rounded-xl py-3 e-shadow inline-flex items-center px-3 mb-5 max-w-sm"
                             :class="darkMode?`bg-button text-gray-100`:`bg-white text-black`">
                          <MessageText
                              :message="message"
                              :isMind="message.is_admin == 0"/>
                        </div>
                      </div>
                      <div v-else
                           class="relative rounded-xl py-3 e-shadow inline-flex items-center px-3 mb-5 max-w-sm"
                           :class="darkMode?`bg-button text-gray-100`:`bg-white text-black`">
                        <MessageText
                            :message="message"
                            :isMind="message.is_admin == 0"/>
                    ...
                    <div>
                      <div v-if="message.reply !== undefined">
                        <div class="flex">
                          <div>
                            <ReplyIcon :size="16"
                                       :fill="darkMode?`#6B7280`:`#4B5563`"></ReplyIcon>
                          </div>
                          <div :class="darkMode?`text-gray-500`:`text-gray-700`"
                               class="text-xs ml-1">
                            {{ senderName(message) }} {{ $t('reply_to') }}
                      ...
                      <div class="relative rounded-xl inline-flex flex-col items-start text-gray-700 max-w-sm">
                        ...
                        <div :class="darkMode?`text-gray-300`:`text-black`"
                             class="text-semibold" v-if="message.content.text">
            ...
            <div class="h-24 flex items-center px-5 relative z-50"
                 ...
                 v-if="replyContact">
              <div class="flex border-l-2 pl-1 mx-10 justify-between w-full items-center"
                ...
                <div>
                  ...
                  <div :class="darkMode?`text-gray-500`:`text-gray-700`"
                       class="text-xs mt-1 flex items-center" v-if="replyContact.content.type === 3">
                ...
                <div :class="darkMode?`bg-button`:`bg-gray-300`"
                  ...
                  <CloseIcon :width="20" :fill="darkMode?`#D1D5DB`:`#000`"/>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- Reply -->
      <div class="h-24 flex items-center px-5 relative z-50"
           :class="darkMode?`bg-secondary text-gray-300`:`bg-white text-primary`" v-if="replyContact">
        <div class="flex border-l-2 pl-1 mx-10 justify-between w-full items-center"
             :class="darkMode?`border-gray-400`:`border-primary`">
          <div>
            <div class="underline">
            ...
            <div :class="darkMode?`text-gray-500`:`text-gray-700`" class="text-xs mt-1 flex items-center"
      ...
      <div class="flex h-24 items-center mb-36 px-5" :class="darkMode?`bg-secondary`:`bg-white e-shadow`"
           style="position:relative;z-index:10" v-if="auth">
          ...
          <div class="cursor-pointer  rounded-full ml-5 mt-2">
            <VueRecord class="record" @result="onResult">
              <div class="w-13 h-13 rounded-full flex items-center justify-center"
                ...
                <mic-icon :size="28" :fill="darkMode?`#4B5563`:`#FFFFFF`"/>
              </div>
    ...
    <div class="fixed w-full h-full top-0 left-0 z-50 flex items-center justify-center bg-black bg-opacity-95"
         v-if="isInfo">
      <div class="w-96 rounded-xl text-sm" :class="darkMode?`bg-secondary text-gray-300`:`bg-gray-50 e-shadow`">
        ...
        <div class="px-5 mt-4">
          <div>
            <input type="text" class="h-12 w-full focus:outline-none placeholder-gray-500"
                   ...
                   :class="darkMode?`bg-youtube pl-3 rounded-md`:`border-b border-gray-300 bg-transparent`"
                   :placeholder="$t('2013')">
          </div>
          <div class="h-3"></div>
          <div>
          ...
          <div class="flex justify-end items-center">
            <button class="focus:outline-none bg-primary px-14 h-11 rounded-md relative text-gray-300"
    ...
    <div class="w-full h-full fixed top-0 left-0 bg-black z-50 flex items-center justify-center bg-opacity-90"
      ...
      <div class="w-96 rounded-lg flex flex-col justify-between"
        ...
        <div class="flex justify-start px-3 w-full relative items-center">
          ...
          <button class="transform mr-5 cursor-pointer focus:outline-none relative -top-2"
    ...
    <!-- Read pdf -->
    <div class="flex justify-center items-center left-0 top-0 fixed bg-black bg-opacity-90 w-full h-full z-50"
         v-if="isRead">
    ...
    <BuyMsg
        v-if="isDelete" :msg="`remove_message`"
