<template>
    <div
        ...
        <div
                class="max-w-sm rounded-xl e-shadow"
                :class="darkMode?`bg-secondary text-gray-400`:`bg-white`">
            ...
            <ul
                    v-if="quickChat.data && quickChat.data.length"
                    class="overflow-y-scroll max-h-96">
                <li
                        v-for="(quickReply, index ) in quickChat.data"
                        :key="index" class="p-3 cursor-pointer"
                        :class="darkMode?`border-t border-black`:`border-t`"
                        @click="quickTextReply(quickReply.text)">
                    {{ quickReply.text }}
                </li>
            </ul>
        </div>
