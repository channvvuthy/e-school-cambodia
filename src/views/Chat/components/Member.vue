<template>
    <div class="flex items-center justify-center items-center fixed left-0 top-0 h-full w-full z-50 bg-black bg-opacity-90">
        <div class="w-1/3" :class="darkMode?`bg-secondary rounded-lg`:`bg-white e-shadow rounded-lg`">
            <div class="flex items-center justify-between h-14 relative px-5 border-b" :class="darkMode?`border-button text-gray-300`:``">
                <div class="flex items-center">
                    <div class="text-base cursor-pointer w-20 text-center h-14 flex items-center justify-center"
                    @click="() => {this.active = 1}"
                     :class="active === 1?`border-b-4 ${darkMode?`border-fb`:`border-primary`}`:``">{{$t('all')}}</div>
                    <div class="text-base cursor-pointer w-20 text-center h-14 flex items-center justify-center"
                    @click="() => {this.active = 2}"
                     :class="active === 2?`border-b-4 ${darkMode?`border-fb`:`border-primary`}`:``">{{$t('admin')}}</div>
                </div>
                    ...
                    <div v-for="(member, index) in members" :key="index">
                        <div class="flex items-center py-3 justify-between" v-if="contact.create_by !== member._id">
                            ...
                            <div class="flex cursor-pointer relative" @click="handleClick(member)" v-if="isAdmin()">
                                ...
                                <div class="w-1 h-1 mx-1 rounded-full" :class="darkMode?`bg-gray-500`:`bg-primary`"></div>
                                <div class="w-1 h-1 rounded-full" :class="darkMode?`bg-gray-500`:`bg-primary`"></div>
                                <div class="absolute right-0 w-56 rounded-lg top-6 z-50 box-active" :class="darkMode?`box-active bg-secondary text-gray-300 border border-button`:`box-active-white bg-white border border-gray-200`" v-if="event === member._id">
                                    <div class="py-3 mx-3" :class="darkMode?`border-b border-button`:`border-b border-gray-200`" @click="confirm(member)">
                                        {{$t('remove')}}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center py-3 justify-between" v-if="contact.create_by === member._id">
                            <div class="flex items-center">
                                ...
                                <div :class="darkMode?`text-gray-300`:``">
                                    ...
                                    <div :class="darkMode?`text-gray-500`:`text-gray-400`" class="text-xs mt-1">{{$t('group_creator')}}</div>
                                </div>
                            </div>
                            <div class="flex cursor-pointer relative" @click="handleClick(member)" v-if="isAdmin(member)">
                                <div class="w-1 h-1 rounded-full" :class="darkMode?`bg-gray-500`:`bg-primary`"></div>
                                <div class="w-1 h-1 mx-1 rounded-full" :class="darkMode?`bg-gray-500`:`bg-primary`"></div>
                                <div class="w-1 h-1 rounded-full" :class="darkMode?`bg-gray-500`:`bg-primary`"></div>
                                <div class="absolute right-0 w-56 rounded-lg top-6 z-50" :class="darkMode?`box-active bg-secondary text-gray-300 border border-button`:`box-active-white bg-white border border-gray-200`" v-if="event === member._id">
                                    <div class="py-3 mx-3" :class="darkMode?`border-b border-button`:`border-b border-gray-200`" @click="leftGroupConfrim(member)">
                                        {{$t('leave')}}
                                    </div>
                                </div>
                            </div>
