  ...
  <div class="flex h-screen m-5 text-sm">
    ...
    <div class="h-full overflow-y-scroll pb-40" :class="darkMode?`bg-secondary`:`bg-white`" @scroll="onScroll"
      ...
      <div class="flex flex-col">
        ...
        <div v-else>
          <!-- Contact List -->
          <div class="flex py-3 px-4 cursor-pointer" v-for="(contact, index) in admin" :key="index"
    ...
    <div class="w-full">
      <div class="flex-1 w-full ml-2 h-screen flex flex-col"
           :class="darkMode?`bg-dark bg-cover`:`bg-light-mode bg-cover`">
    ...
    <QuickChat
        v-if="isQuickChat"
        @closeQuickReply="()=>{this.isQuickChat = false}"
        @quickTextReply="quickTextReply($event)"/>
    <!-- Post detail -->
    <PostDetail
        @dismiss="()=>{this.isChatDetail = false}"
        :post="chatDetail"
        v-if="isChatDetail"/>
    <!-- Video detail -->
    <VideoDetail
...
import PostDetail from "../Video/components/PostDetail";
