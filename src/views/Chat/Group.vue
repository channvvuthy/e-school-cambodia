    ...
    <div class="flex h-screen m-5 text-sm">
        <div class="w-80 h-full overflow-y-scroll pb-40" :class="darkMode?`bg-secondary text-gray-300`:`bg-white`"
            ...
            <div class="relative mb-4 px-3">
                <input type="text" v-on:keyup.enter="searchContact" v-model="searchQuery"
            ...
            <div class="flex flex-col">
                ...
                <div v-else>
                    <!-- Contact List -->
                    <div class="flex items-center py-3 px-4 cursor-pointer" v-for="(contact, index) in contacts"
                         :key="index"
                        ...
                        <div>
                            ...
                            <div class="text-xs font-normal" :class="darkMode?`text-gray-500`:`text-gray-400`">
                                {{contact.last == undefined?$t('ticket') + ' ' +
                                contact.ticket:cutString(contact.last.message, 20)}}
                            </div>
                        </div>
                        <div class="flex flex-1 justify-end items-end h-full">
                            <div>
                                <div class="flex justify-center" v-if="contact.unread">
                                    <div class="h-4 w-4 rounded-full flex items-center justify-center text-xs"
                                         :class="darkMode?`bg-white text-black`:`bg-heart text-white`">
                                        {{contact.unread}}
                                    </div>
                                </div>
                                <div class="text-xs mt-1" :class="darkMode?`text-gray-500`:``">
                                    {{contact.last == undefined?$t('unread') + ' ' +
                                    contact.unread:formatTime(contact.last.date)}}
                                </div>
                            </div>
            ...
            <div :class="isNext?`my-2`:`m-5`">
                <template v-if="isNext">
                    <div class="flex shadow px-5 py-4 items-center" :class="darkMode?`bg-secondary`:`bg-white`">
                        ...
                        <div>
                            <div class="text-xs" :class="darkMode?`text-gray-300`:`text-gray-400`">
                                {{$t('name_your_group')}}
                            </div>
                            <div>
                                <input type="text" class="bg-transparent focus:outline-none text-smm py-2 font-semibold"
                                       :placeholder="$t('group_name')" v-model="groupName" ref="groupName">
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <div v-if="loadingFriend" class="flex items-center justify-center"
                ...
                <template v-if="isNext">
                    <div class="mt-2 pb-5 h-screen" :class="darkMode?`bg-secondary`:`bg-white`">
                        ...
                        ​
                        <div>
                            <div v-for="(member, index) in selectedMember" :key="index" class="py-3  border-b px-5"
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs font-normal"
                                             :class="darkMode?`text-gray-500`:`text-gray-400`">Online
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="h-screen pb-96 overflow-y-scroll flex flex-col mt-2" @scroll="onScroll">
                        <div v-for="(friend, index) in friends.list" :key="index">
                            <div class="flex items-center justify-between border-b py-3"
                                ...
                                <div class="flex items-center">
                                    <div class="h-14 w-14 rounded-full bg-cover bg-gray-300"
                                ...
                                <div>
                                    <div class="h-5 w-5 rounded-full flex items-center justify-center relative"
                                        ...
                                        <label class="absolute cursor-pointer w-full h-full items-center justify-center flex flex-col">
                                            <input type="checkbox" :value="friend._id"
                                                   @click="selectMember($event, friend)" class="hidden">
