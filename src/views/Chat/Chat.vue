    ...
    <div class="flex h-screen m-5">
        <div class="w-80 h-full overflow-y-scroll pb-40" :class="darkMode?`bg-secondary`:`bg-white`">
            <div class="flex px-4 py-2 items-center justify-between relative" :class="darkMode?`text-gray-300`:`bg-white`">
                ...
                <div class="cursor-pointer" @click="() => {this.eTalkOption = true; this.$refs['eTalkOption'].focus()}">
                ...
                <!-- eTalk option -->
                <div :class="darkMode?`bg-button`:`bg-white`" class="rounded-md overflow-hidden e-shadow absolute right-5 top-14 z-50 cursor-pointer" v-if="eTalkOption">
            ...
            <input type="text" ref="eTalkOption" @blur="() =>{this.eTalkOption = false}" class="absolute focus:outline-none h-0">
            <input type="text" ref="chatOption" @blur="() =>{this.chatOption = false}" class="absolute focus:outline-none h-0">
            <!-- Search -->
            ...
            <div class="flex flex-col">
                ...
                <div v-else>
                    <div class="flex items-center py-3 px-4 cursor-pointer" v-for="(contact, index) in resultQuery" :key="index" :class="darkMode?`border-b border-black ${active === index ?`bg-button`:``}`:`border-b ${active === index?`bg-blue-100`:``}`">
                        <div>
        ...
        <div class="flex-1 w-full ml-2 h-screen flex flex-col" :class="darkMode?`bg-youtube`:`bg-img-primary`">
            <div :class="darkMode?`bg-secondary text-gray-300`:`bg-white`" class="px-4 py-3 flex text-sm items-center shadow relative">
                ...
                <div class="flex-1 flex flex-col items-end cursor-pointer" @click="() => {this.chatOption = true;this.$refs['chatOption'].focus()}">
