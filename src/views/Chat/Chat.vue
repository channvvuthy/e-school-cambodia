<template>
  <div class="flex h-screen m-5 text-sm">
    ...
    <div class="h-full overflow-y-scroll pb-40" :class="darkMode ? `bg-secondary` : `bg-white`" @scroll="onScroll"
      style="width: 520px">
      <div class="flex px-4 py-2 items-center justify-between relative"
        ...
        <!-- eTalk option -->
        <div :class="darkMode ? `bg-button` : `bg-white`" class="
            rounded-md
            overflow-hidden
            e-shadow
            absolute
          ...
          <div class="px-8 h-12 flex items-center justify-center border-b"
            :class="darkMode ? `border-youtube` : `border-gray-200`" @click="goTo('network')">
            {{ $t("connect_friend") }}
          </div>
          <div class="px-8 h-12 flex items-center justify-center border-b"
          ...
          <div class="px-8 h-12 flex items-center justify-center border-b"
            :class="darkMode ? `border-youtube` : `border-gray-200`" @click="goTo('hot-chat')">
            {{ $t("need_support") }}
          </div>
          <div class="px-8 h-12 flex items-center justify-center"
            :class="darkMode ? `border-youtube` : `border-gray-400`" @click="muteAll()">
            {{ $t(muteLabel) }}
          </div>
        </div>
        <!-- Overlay -->
        <div class="fixed w-full h-full left-0 top-0 z-50" v-if="eTalkOption || chatOption" @click="
          () => {
      ...
      <div class="relative mb-4 px-3">
        <input type="text" v-model="searchQuery" class="w-full rounded-md h-10 focus:outline-none pl-3"
          @keyup.enter="searchContact" :class="darkMode ? `bg-youtube text-gray-300` : `bg-softGray`"
          :placeholder="$t('1001')" />
      ...
      <div class="flex flex-col">
        ...
        <div v-else>
          ...
          <div class="flex py-3 px-4 cursor-pointer" v-for="(contact, index) in contacts" :key="index"
            @click="selectedContact(contact, index)" :class="
              darkMode
            ...
            <div>
              <div class="
                  ...
                  mr-3
                  bg-center
                " :style="{ backgroundImage: `url(${contactProfile(contact)})` }"></div>
            </div>

            <div class="flex justify-between flex-1 items-center" :class="darkMode ? `text-gray-300` : ``">
              <div>
                <div class="font-PoppinsMedium flex space-x-1">
                  <div>
                    {{ contactName(contact) }}
                  </div>
                  <span v-if="contact.type == 'ads'" class="
                      text-xs
                      font-black
                      text-center
                      h-4
              ...
              <div v-else-if="contact.type == 'chat'">
                <div class="flex flex-col justify-end items-end">
                  <div class="flex justify-center" v-if="contact.chat && contact.chat.unread">
                    <div class="
                        notification
                        h-4
                        w-4
      ...
      <div class="flex-1 w-full ml-2 h-screen flex flex-col bg-cover" :class="darkMode ? `bg-dark` : `bg-light-mode`">
        <div :class="darkMode ? `bg-secondary text-gray-300` : `bg-white`"
          ...
          <div class="
              ...
              justify-center
              bg-center
            " :style="{ backgroundImage: `url(${contact.photo})` }" @click="contactPhoto()" :class="
              contact.type === 0 || this.contact.type === 1
                ? ``
                : `cursor-pointer`
            ">
          ...
          <div class="flex justify-end items-center flex-1" v-if="contact.type == 100 && !isExpired">
            <div class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 h-10 flex
                      items-center
                      justify-center
                      text-white
                      rounded-full
          ...
          <div :class="darkMode ? `bg-button` : `bg-white`" class="
            ...
            <template v-if="contact.type === 1">
              ...
              <div class="px-8 h-12 flex items-center border-b" @click="userDetail(user)"
                :class="darkMode ? `border-youtube` : `border-gray-200`">
                {{ $t("profile") }}
              </div>
              <div class="px-8 h-12 flex items-center border-b"
            ...
            <template v-if="contact.type === 2">
              ...
              <div class="px-8 h-12 flex items-center border-b relative"
                :class="darkMode ? `border-youtube` : `border-gray-200`" @click="shareLink">
                {{ $t("copy_link") }}
              </div>
            </template>
          </div>
        ...
        <div class="flex-1 h-full flex flex-col pb-36 py-5">
          ...
          <div class="flex-1 overflow-y-scroll h-full" ref="feed" @scroll="getMoreMessage">
            ...
            <ul class="px-5 pr-14 pb-20" id="box relative">
              <li v-for="(message, index) in messages" :key="index" :id="message._id" @contextmenu="showReply(message)"
                class="relative">
                <div class="absolute w-full justify-center flex" v-if="replyId._id === message._id"
                  style="z-index: 100">
                  <Reply :contact="contact" :message="message" @deleteMessage="dlMessage" @copy="copy()" @save="save()"
                    @reply="reply()" />
                </div>
                <!--  Message text -->
                <template v-if="
                  ...
                  " class="items-center relative">
                    ...
                    <div>
                      <div v-if="message.reply !== undefined">
                        <div class="flex">
                          <div>
                            <ReplyIcon :size="16" :fill="darkMode ? `#6B7280` : `#374151`" />
                          </div>
                          <div :class="
                        ...
                        <div class="
                          ...
                          " :class="
                            darkMode
                              ? `bg-button text-gray-300 ${message.content.type == 0
                                ? `bg-opacity-40 rounded-3xl`
                                : `rounded-xl e-shadow`
                              }`
                              : `bg-white ${message.content.type == 0
                                ? `bg-opacity-20 rounded-3xl `
                                : `rounded-xl e-shadow`
                              }`
                          ">
                          <MessageText :message="message" :isMind="auth === sender(message)" />
                        </div>
                      </div>
                      <div v-else class="
                          relative
                  ...
                  " class="items-center relative">
                    ...
                    <div>
                      ...
                      <div v-if="message.reply !== undefined">
                        <div class="flex">
                          <div>
                            <ReplyIcon :size="16" :fill="darkMode ? `#6B7280` : `#374151`"></ReplyIcon>
                          </div>
                          <div :class="
                            darkMode ? `text-gray-500` : `text-gray-700`
                          " class="text-xs ml-1">
                      ...
                      <div class="
                          ...
                          max-w-sm
                        ">
                        <audio controls class="focus:outline-none" controlsList="nodownload">
                  ...
                  " class="items-center relative mb-2">
                    ...
                    <div>
                      ...
                      <div class="
                          ...
                          max-w-sm
                        ">
                        <img class="max-w-xs rounded-md mb-2 cursor-pointer" :src="message.content.file.url"
                          @click="previewImage(message.content.file.url)" />
                        <div :class="darkMode ? `text-gray-300` : `text-black`" class="text-semibold"
          ...
          <!-- Reply -->
          <div class="h-24 flex items-center px-5 relative z-40" :class="
            darkMode
          ...
          " v-if="replyContact">
            <div class="
              ...
              <div>
                ...
                <div :class="darkMode ? `text-gray-500` : `text-gray-700`" class="text-xs"
                ...
                <div :class="darkMode ? `text-gray-500` : `text-gray-400`" class="text-xs mt-1 flex items-center"
              ...
              <div :class="darkMode ? `bg-button` : `bg-gray-300`" class="
                  cursor-pointer
                  rounded-full
                  w-7
                  h-7
          ...
          ">
            ...
            <template v-if="!isTypeExam">
              <div class="
                ...
                <ul>
                  <li v-for="(mention, key) in mentions" :key="key"
                    class="py-1 cursor-pointer h-12 flex items-center px-3" :class="
                      darkMode
                    ...
                    <div class="
                        ...
                        bg-cover bg-gray-300
                        mr-3
                      " :style="{ backgroundImage: `url(${mention.photo})` }"></div>
              ...
              <textarea class="
                  ...
                  py-4
                  px-5
                " :disabled="contact.block_by || !isSelectedContact" @keydown="enableWatch" name="message"
                ref="message" v-model="message.text" @keyup.enter.exact="onMessage" :placeholder="$t(`2112`)" :class="
            ...
            <template v-else>
              ...
              <input type="file" ref="upload" accept="image/*" class="hidden" @change="onSelectFile" />
            </template>
          </div>
          ...
          <div class="
              ...
              bg-opacity-90
            " v-if="isPreview">
            <div class="w-96 rounded-lg flex flex-col justify-between" :class="
              darkMode ? `bg-secondary text-gray-300` : `bg-white shadow`
            ">
              <div class="py-4 px-5 relative font-PoppinsMedium">
                {{ $t("preview") }}
                <div class="absolute right-3 top-3 cursor-pointer" @click="
                  () => {
              ...
              <div class="h-3"></div>
              <div class="flex px-3 w-full relative items-center" :class="isTypeExam ? `justify-end` : `justify-start`">
                <input v-if="!isTypeExam" type="text" placeholder="Add a caption..."
                  class="w-full py-2 focus:outline-none pl-2" v-model="message.text"
                ...
                <template v-if="!isTypeExam">
                  <button :disabled="sending" @click="sendFile()" style="background-color: rgba(5, 81, 116, 0.2)"
                ...
                <template v-else>
                  <button :disabled="sending" @click="sendFile()" :class="sending ? `bg-opacity-70` : ``" class="
                      w-full
                      h-12
                      px-6
                      rounded-lg
    ...
    <div class="fixed bg-black w-full h-full left-0 top-0 bg-opacity-70" style="z-index: 50" v-if="replyId" @click="
      () => {
        this.replyId = false;
      }
    "></div>
    <Rename v-if="isRename" @cancelRename="
      () => {
        this.isRename = false;
      }
    " :group="contact">
    </Rename>
    <Member v-if="isMember" @closeMember="
      () => {
        this.isMember = false;
      }
    " :contact="contact" @adminLeft="adminLeft">
    </Member>
    <BuyMsg v-if="isConfirm" @cancelModal="
      () => {
        this.isConfirm = false;
      }
    " :msg="msg" @yes="leaveGroup()" />
    <BuyMsg v-if="isDisconnect" @cancelModal="
      () => {
        this.isDisconnect = false;
      }
    " :msg="`block_contact`" @yes="disconnect()" />
    <BuyMsg v-if="isUnblock" @cancelModal="
      () => {
        this.isUnblock = false;
      }
    " :msg="`unblock_contact`" @yes="unblock()" />
    <BuyMsg v-if="isDelete" :msg="`remove_message`" @cancelModal="
      () => {
        this.isDelete = false;
      }
    " @yes="confirmDelete" />
    <AdminMember v-if="showAdminMember" @closeAdminMember="
      () => {
        this.showAdminMember = false;
      }
    " @selectedChat="selectedChat($event)" />
    <PreviewImage v-if="viewChat" :URL="previewUrl" @closePreviewImage="
      () => {
        this.viewChat = false;
      }
    " />
    ...
    " :post="postDetail" v-if="isPostDetail" />
    <!-- Video detail -->
    <VideoDetail @dismiss="
      () => {
        this.isVideo = false;
      }
    " :post="postDetail" v-if="isVideo" />
    <Modal @dismiss="
      () => {
        this.loadingAds = false;
      }
    " v-if="loadingAds" width="20" :is-transparent="true">
      <LoadingWhite />
    </Modal>
  </div>
</template>
        ...
        this.getMessage({
          ...
          if (contact.type == 100) {
            ...
            this.startingExam = setInterval(() => {
              this.examMilliseconds = this.examMilliseconds - this.startCounting;
