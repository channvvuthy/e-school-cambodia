    ...
    <div class="h-full overflow-y-scroll pb-40" :class="darkMode?`bg-secondary`:`bg-white`" @scroll="onScroll"
      ...
      <div class="flex px-4 py-2 items-center justify-between relative" :class="darkMode?`text-gray-300`:`bg-white`">
        ...
        <div :class="darkMode?`bg-button`:`bg-white`"
             class="rounded-md overflow-hidden e-shadow absolute right-5 top-14 cursor-pointer" v-if="eTalkOption"
             style="z-index:55">
    ...
    <div class="w-full">
      <div class="flex-1 w-full ml-2 h-screen flex flex-col bg-cover" :class="darkMode?`bg-dark`:`bg-light-mode`">
        <div :class="darkMode?`bg-secondary text-gray-300`:`bg-white`"
          ...
          <div
              class="h-12 w-12 rounded-full shadow bg-cover bg-gray-300 mr-3 flex items-center justify-center bg-center"
              :style="{backgroundImage:`url(${contact.photo})`}" @click="contactPhoto()"
              :class="(contact.type === 0 || this.contact.type === 1 )?``:`cursor-pointer`">
          ...
          <!-- eTalk option -->
          <div :class="darkMode?`bg-button`:`bg-white`"
               class="z-50 rounded-md overflow-hidden e-shadow absolute right-5 top-20 z-50 cursor-pointer"
               v-if="chatOption">
            ...
            <template v-if="contact.type === 2">
              ...
              <div class="px-8 h-12 flex items-center border-b" :class="darkMode?`border-youtube`:`border-gray-200`"
                   @click="() => {this.isRename = true; this.chatOption = false;}">
                {{ $t('rename_group') }}
              </div>
              <div class="px-8 h-12 flex items-center border-b" :class="darkMode?`border-youtube`:`border-gray-200`"
              ...
              <div class="px-8 h-12 flex items-center border-b" :class="darkMode?`border-youtube`:`border-gray-200`"
                   @click="() => {this.$router.push({name: 'add-member', params:{contact}})}">
                {{ $t('add_member') }}
              </div>
              <div class="px-8 h-12 flex items-center border-b" :class="darkMode?`border-youtube`:`border-gray-200`"
                   @click="() => {this.isMember = true; this.chatOption = false}">
                {{ $t('member') }}
              </div>
        ...
        <div class="flex-1 h-full flex flex-col pb-36 py-5">
                    ...
                    <div>
                      ...
                      <div class="relative rounded-xl inline-flex flex-col items-start text-black max-w-sm">
                        ...
                        <div :class="darkMode?`text-gray-300`:`text-black`" class="text-semibold"
                             v-if="message.content.text">{{ message.content.text }}
                        </div>
                      </div>
                      <div class="flex items-center" v-if="auth !== sender(message)">
                        <div :class="darkMode?`text-gray-500`:`text-gray-700`" class="text-xs whitespace-nowrap">
          ...
          <!-- Reply -->
          <div class="h-24 flex items-center px-5 relative z-40"
               :class="darkMode?`bg-secondary text-gray-300`:`bg-white e-shadow text-primary`" v-if="replyContact">
          ...
          <div class="h-24 flex items-center px-5 relative z-40"
            ...
            <div class="h-48 overflow-y-scroll w-48 rounded-lg absolute bottom-28 left-0 mb-2"
                 :class="darkMode?`bg-secondary text-gray-300`:`bg-white e-shadow`" v-if="showMention">
              <ul>
                <li v-for="(mention, key) in mentions" :key="key"
                  ...
                  <div class="h-8 w-8 rounded-full shadow bg-cover bg-gray-300 mr-3"
