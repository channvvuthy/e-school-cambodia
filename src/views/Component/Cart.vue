  ...
  <div>
    <div class="fixed w-full h-full left-0 top-0 flex justify-center items-center text-sm bg-black bg-opacity-90"
      ...
      <div :class="darkMode?`bg-secondary text-gray-300`:`bg-white`"
           class="rounded-xl shadow relative py-3 w-4/12">
        ...
        <div class="overflow-y-scroll max-h-100">
          ...
          <div :class="darkMode?`bg-youtube`:`bg-softGray`" v-if="carts.list && receipts.length">
            ...
            <template v-else>
              <div class="px-5 py-3"
                   :class="darkMode?`border-t border-secondary`:`border-t border-gray-300`"
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <div class="flex flex-col ml-2"
                        ...
                        <div class="transform -rotate-180 cursor-pointer">
                          <ChevronIcon
                              :size="18"
                              :fill="darkMode?`${cart.price.month?`#D1D5DB`:`#4B5563`}`
                                                            :`${cart.price.month?`#000000`:`#c0272d`}`"/>
                        </div>
          ...
          <div class="border-t" :class="darkMode?`border-button`:``">
            <div class="my-7 flex justify-center items-center flex-col ">
              <div class="mb-5 cursor-pointer"
                   :class="darkMode?`text-gray-300`:`text-youtube`"
              ...
              <div class="mb-5 cursor-pointer"
                   :class="darkMode?`text-gray-300`:`text-youtube`"
                   @click="confirmRemoveCoupon"
                   v-else>
                {{ $t('delete_coupon') }}
              </div>
              <button
                  class="focus:outline-none rounded-lg shadow px-32 h-12"
                  :class="darkMode?`bg-primary text-white`:`bg-primary text-white`"
                  @click="pay">
                ${{ totalCart }} {{ $t('2303') }}
              </button>
            </div>
