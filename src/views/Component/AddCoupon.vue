<template>
  <div class="fixed w-full h-full left-0 top-0 z-50 bg-black bg-opacity-90 flex items-center justify-center"
    ...
    <div :class="darkMode?`bg-secondary text-gray-300`:`bg-white`" class="rounded-xl">
      ...
      <div class="px-5">
        <input type="text" class="focus:outline-none border h-9 mt-3 rounded-md w-full pl-3 font-normal py-2"
               :class="darkMode?`bg-youtube border-active`:``" ref="coupon" id="coupon" v-model="coupon">
      </div>
      <div class="flex justify-between items-center px-5 h-10 text-sm border-t mt-4"
           :class="darkMode?`text-white border-button`:`text-primary`">
