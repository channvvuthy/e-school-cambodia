    ...
    <div v-if="isEdit" class="m-5 rounded-lg p-5 w-3/4 z-10" :class="darkMode?`text-gray-300`:`bg-white e-shadow`">
      ...
      <div class="mt-5">
        ...
        <div class="grid grid-cols-2 gap-5">
          <div>
            <label>
              <div>{{ $t('2013') }}</div>
              <div class="h-2"></div>
              <input type="text" ref="first_name" class="border focus:outline-none h-10 rounded-md w-full pl-2"
          ...
          <div>
            <label>
              <div>{{ $t('2014') }}</div>
              <div class="h-2"></div>
              <input type="text" ref="last_name" class="border focus:outline-none h-10 rounded-md w-full pl-2"
          ...
          <div>
            <label>
              ...
              <select class="w-full border h-10 rounded-md focus:outline-none"
                      :class="darkMode?`bg-secondary border-button`:`bg-softGray border-gray-300 `" ref="gender"
        ...
        <div class="grid grid-cols-2 gap-5 mt-6">
          <div>
            <label>
              <div>{{ $t('2124') }}</div>
              <div class="h-2"></div>
              <select class="w-full border h-10 rounded-md focus:outline-none" id="provinces" @change="onChange($event)"
                      :class="darkMode?`bg-secondary border-button`:`bg-softGray border-gray-300 `">
          ...
          <div class="relative w-full">
            ...
            <ul class="w-full border mt-1 rounded overflow-y-scroll absolute  left-0 z-50"
                :class="darkMode?`border-button bg-secondary `:`bg-softGray`" v-if="showSchool && schools.length"
                style="z-index:100;height:300px;">
              <li v-for="(school,index) in schools" :key="index"
                  class="h-10 px-3 flex items-center justify-start cursor-pointer" @click="takeSchool(school)">
                {{ school.name }}
              </li>
            </ul>
          </div>
        ...
        <div class="flex justify-end items-center mt-6 mb-2">
          ...
          <button class="h-12 rounded-md bg-primary focus:outline-none px-5 text-white relative" @click="updateProfile"
