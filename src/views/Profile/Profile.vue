    ...
    <div class="h-screen">
            ...
            <div class="flex mt-5 px-5">
                ...
                <input type="file" ref="photo" class="hidden" @change="onSelectedPhoto">
                    <div class="h-full w-full absolute top-0 left-0 flex flex-col items-center justify-center">
                        <div class="w-32 h-32 rounded-full bg-gray-300 bg-cover bg-center flex items-center justify-center relative cursor-pointer" :style="{backgroundImage:`url(${stProfile.photo})`}"
        ...
        <div v-if="isEdit" class="m-5 rounded-lg p-5 w-3/4 z-10" :class="darkMode?`text-gray-300`:`bg-white e-shadow`">
           ...
           <div class="flex items-center ">
               <div class="w-20 h-20 rounded-full bg-primary bg-cover bg-center cursor-pointer relative" :style="{backgroundImage:`url(${stProfile.photo})`}" 
               @click="()=>{this.$refs.photo.click()}" @mouseover="() =>{this.isPic = true}" @mouseleave="() => {this.isPic = false}">
                <div class="bg-gradient-to-t pb-2 from-black absolute w-full h-full rounded-full flex items-end justify-center" v-if="isPic">
                    <CameraIcon fill="#FFFFFF"></CameraIcon>
                </div>
               </div>
           ...
           <div class="mt-5">
                ...
                <div class="flex justify-end items-center mt-6 mb-2">
                    <button class="mr-5 h-12 rounded-md bg-secondary focus:outline-none px-5 text-white"  @click="()=>{this.isEdit = false}">
                         <span>{{$t('1011')}}</span>
                    </button>
                    <button class="h-12 rounded-md bg-primary focus:outline-none px-5 text-white relative" @click="updateProfile" :disabled="(noUpdate || updating)" :class="(noUpdate || updating)?`bg-opacity-60`:``">
