<template>
    <div class="absolute left-0 top-0 w-full h-full z-50 pt-10 px-10"
        ...
        <div class="h-screen overflow-y-scroll pb-72 mt-10">
            <div class="grid grid-cols-3 gap-10 text-sm">
                <div v-for="(quizzes, index) in quiz.list" :key="index">
                    <div :class="darkMode?`bg-secondary shadow-md`:`bg-gray-50`"
                         class="rounded-xl shadow-md p-5 my-scroll view overflow-x-scroll"
                        ...
                        <div v-for="(list,key) in quizzes.check_list" :key="key">
                            <div class="inline-flex items-center mb-5 relative"
                                 :class="correctAnswer(list._id)?`text-pass`:answer(list._id)?`delete`:``">
                                <div class="h-5 w-5 rounded border mr-5 relative"
                                     :class="correctAnswer(list._id)?`active-checkbox border-pass`:answer(list._id)?`incorrect`:``"></div>
            ...
            <div class="m-auto flex justify-center mt-10">
                <button class="focus:outline-none px-40 py-2 rounded-md"
