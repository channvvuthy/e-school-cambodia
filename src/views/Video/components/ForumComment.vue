  ...
  <div class="ml-5 flex flex-col justify-between border border-t-0"
    ...
    <template v-else>
        ...
        <div :class="darkMode ?`bg-youtube`: `bg-forum`" class="py-5 px-4">
          <div class="flex">
            ...
            <div class="text-primary ml-4">
              <div :class="darkMode?`text-gray-300`:``" class="font-bold">{{ comments.forum.user.name }}</div>
      ...
      <div v-if="sticker._id"
        ...
        <div class="w-24 flex items-center justify-center rounded-lg relative p-2" :class="darkMode ? `` : `bg-forum`">
          <div
              @click="()=>{this.sticker ={};}"
              class="absolute cursor-pointer -right-2 -top-2 w-5 h-5 bg-primary flex items-center justify-center shadow rounded-full">
            <CloseIcon :width="18" fill="#fff"></CloseIcon>
          </div>
      ...
      >
        <div class="h-12 w-12 cursor-pointer">
          <div
            ...
            <ImageIcon :fill="darkMode?`#909090`:`#055174`" :size="25"></ImageIcon>
          </div>
        </div>
        ...
        <div class="mr-5 cursor-pointer" @click="()=>{this.isSticker =! this.isSticker}">
          <SmileEmoji :size="45" :fill="darkMode ?`#909090`: `#979797`"></SmileEmoji>
        </div>
        <div class="h-12 w-12 cursor-pointer">
          <div
    ...
    <template v-if="isSticker">
      <Sticker @selectSticker="selectSticker($event)"></Sticker>
    </template>
  </div>
</template>
<script>
