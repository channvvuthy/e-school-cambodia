        ...
        <div class="flex-1 flex flex-col justify-between">
          ...
          <div class="flex justify-between items-center text-sm items-center">
            <div class="flex">
              ...
              <div class="mx-2" :class="darkMode?list.order === order?`text-black`:`text-byline`:`opacity-50`">
                {{ kFormatter(list.view) }}
              </div>
              <div :class="darkMode?list.order === order?`text-youtube`:`text-byline`:`opacity-50`">{{ $t('1003') }}
              </div>
            </div>
            <div class="flex">
              ...
              <template v-if="canWatch(list.free_watch)">
                <div class="mr-5 cursor-pointer"
                     @click="list.is_favorite?removeMyFavorite(list._id):addFavorite(list._id)">
                  <FavoriteFill v-if="list.is_favorite" :size="20"
                                :fill="darkMode?list.order === order?`#c0272d`:`#E5E7EB`:`#c0272d`"></FavoriteFill>
                  <template v-else>
                    <FavoriteIcon :size="20" v-if="list.order === order"
                                  :fill="darkMode?`#181818`:`#4A4A4A`"></FavoriteIcon>
                ...
                <div class="cursor-pointer">
                  ...
                  <template v-else>
                    <div @click="showOptionDownload(list)">
                      <DownloadIcon :size="18" v-if="list.order != order"
                                    :fill="darkMode?`#E5E7EB`:`#4A4A4A`"></DownloadIcon>
              ...
              <div v-else>
                <LockIcon :width="20" :height="20"
                          :fill="(list.order != order && darkMode)?`#E5E7EB`:darkMode?`#E5E7EB`:`#4A4A4A`"></LockIcon>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="w-80 rounded-xl shadow" :class="darkMode?`bg-secondary text-gray-300`:`bg-white`">
        ...
        <ul>
          <li class="w-full text-center h-12 leading-11 flex items-center justify-center border-b cursor-pointer"
              :class="darkMode?`border-button`:`border-gray-300`" @click="download(360)"><span>360p</span></li>
          <li class="w-full text-center h-12 leading-11 flex items-center justify-center border-b cursor-pointer"
              :class="darkMode?`border-button`:`border-gray-300`" @click="download(720)"><span>720p</span></li>
        </ul>
        <div class="flex items-center justify-center py-3 cursor-pointer" :class="darkMode?`text-pass`:`text-heart`"
