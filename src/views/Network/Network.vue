  ...
  <div>
    <div class="flex items-center h-14 text-sm px-5 mt-3"
      ...
      <div class="mx-10 cursor-pointer"
           :class="active === `friend_request`?`font-bold ${darkMode?`text-fb`:`text-primary`}`:``"
           @click="switchMenu('friend_request')">{{ $t('friend_request') }}
      </div>
      <div class="cursor-pointer cursor-pointer"
           :class="active === `add_friend`?`font-bold ${darkMode?`text-fb`:`text-primary`}`:``"
           @click="switchMenu('add_friend')">{{ $t('add_friend') }}
      </div>
      <div class="ml-10 cursor-pointer"
           :class="active === `your_request`?`font-bold ${darkMode?`text-fb`:`text-primary`}`:``"
           @click="switchMenu('your_request')">{{ $t('your_request') }}
      </div>
    </div>
      ...
      <div class="w-4/6 rounded-xl p-5 overflow-y-scroll" :class="darkMode?`bg-secondary`:`bg-white shadow`"
        ...
        <div class="relative w-3/4 sticky top-0">
          <input type="text" class="focus:outline-none h-10 rounded-md bg-softGray pl-3 text-sm w-full"
                 :placeholder="$t('1001')" :class="darkMode?`bg-button text-gray-300`:``" v-on:keyup.enter="search"
                 v-model="s">
