<template>
  <div id="video_player_box" class="flex-cols start items-center w-full relative">
    <template v-if="LoadingWhiteSuccess">
      <div
          class="absolute top-0 left-0 w-full h-full flex justify-center items-center text-center z-20 bg-black bg-opacity-0"
          v-if="loadingVideo" :style="{height:video.height+'px'}">
      </div>
      <div class="bg-black">
        <video id="my_download" class="m-auto w-full" ref="my_download"
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex justify-start items-center px-5 relative">
            <div class="mr-5">
              ...
              <!--Show option-->
              <div
                  class="absolute bg-white text-black text-14px font-khmer_os w-44 text-center right-5 -top-46 rounded shadow">
                <ul v-if="showOption">
              ...
              <!--Show speed-->
              <div
                  class="absolute bg-white text-black text-14px font-khmer_os w-44 text-center right-5 -top-64 rounded text-blue-500 shadow">
                <ul v-if="showSpeed">
                  <li class="border border-l-0 border -l-0 border-r-0 border-t-0 border-gray-200 h-10 leading-10 font-13px text-gray-500">
                    ល្បឿន
                  </li>
              ...
              <div
                ...
                <ul v-if="showQuality">
                  ...
                  <li class="cursor-pointer border border-l-0 border -l-0 border-r-0 border-t-0 border-gray-200 h-9 leading-9 text-custom "
                      @click="changeQuality()">
                    ឯកសារទាញយក
                  </li>
                  <li class="cursor-pointer h-10 leading-10 text-red-500 font-semibold"
...
//                this.lastWatchVideo()
    ...
    onEnded() {
      this.seekSlider.style.background = `linear-gradient(90deg, rgb(255, 14, 9) 100%, rgb(214,214,214) 100%)`
      this.showOption = false
      this.lastWatchVideo()
    ...
    timeUpdate() {
      ...
      this.seekSlider.style.background = `linear-gradient(90deg, rgb(255, 14, 9) ${nt}%, rgb(214,214,214) ${nt}%)`
    ...
    getVideo() {
      ...
      let interval = setInterval(() => {
        if (document.getElementById("my_download") !== null) {

          clearInterval(interval)
          this.vid = document.getElementById("my_download")
