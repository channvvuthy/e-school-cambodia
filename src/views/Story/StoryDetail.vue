...
<div class="
    ...
    storyDetail.user.name
         ...
         <div class="flex-1 flex items-center justify-center overflow-hidden" style="max-height: 80%">
             <img :src="storyDetail.photo.url ? storyDetail.photo.url : '/poster.png'" style="max-height: 100%" class="story-profile" />
         </div>
         <div class="h-4"></div>
         <!-- Viewer -->
         <div class="
    ...
    $t("1004")
                 ...
                 <span>
                     <ChevronIcon :fill="color" :size="18" /></span>
             </div>
         </div>
         <!-- End viewer -->
         <!-- List viewer -->
         <div :class="darkMode ? 'bg-secondary text-textSecondary' : 'bg-white'" class="
           absolute
           z-50
           left-0
           w-full
           ...
           rounded-xl
           shadow-md
         " @scroll="onScroll" v-if="showViewer">
             <div class="top-0 sticky pt-14 pb-4" :class="darkMode ? `bg-secondary` : `text-black bg-white`">
                 <div class="absolute right-3 top-0 cursor-pointer opacity-70 z-50" @click="
               () => {
             ...
             ">
                 ...
                 <div class="flex ml-5 text-sm font-semibold items-center">
                     <span>
                         <Eye :fill="darkMode ? '#ffffff' : '#000000'" /></span>
                     <span class="px-2" :class="darkMode ? `text-white` : `text-black`">
                         {{
    storyDetail.view
  }}</span>
                     <span class="pr-1" :class="darkMode ? `text-white` : `text-black`">{{
    $t("1004")
  }}</span>
                 </div>
             </div>
             <div class="mt-5">
                 <div v-for="(viewer, index) in storyDetail.viewer" :key="index" class="text-black text-xs font-semibold">
                   ...
                   h-12
                   rounded-full
                   bg-cover bg-gray-200
                   mr-5
                   bg-center
                   cursor-pointer
                   z-50
                 " :style="{
                   backgroundImage: `url(${viewer.photo || 'poster.png'})`,
                 }" @click="getUser(viewer._id)"></div>
                         <div :class="darkMode ? 'text-textSecondary' : ''">
                             {{
    viewer.name
  }}
                         </div>
...
export default {
  ...
  methods: {
    ...
    averageColor() {
      fac
          .getColorAsync(document.querySelector(".story-profile"))
          .then((color) => {
