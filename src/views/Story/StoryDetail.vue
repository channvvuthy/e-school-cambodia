        ...
        <div class="md:w-96 2xl:w-100 h-85 rounded-lg flex flex-col justify-between text-white relative"
            ...
            <div class="flex items-center left-3 justify-start p-4" v-if="storyDetail.user">
                <div class="w-12 h-12 rounded-full bg-cover mr-3 bg-center cursor-pointer z-50"
                     :style="{backgroundImage:`url(${storyDetail.user.photo})`}"
                     @click="getUser(storyDetail.user._id)"></div>
            ...
            <div class="flex-1 flex items-center justify-center overflow-hidden" style="max-height:80%;">
                <img
                        :src="storyDetail.photo.url?storyDetail.photo.url:'/poster.png'"
                        style="max-height:100%;" class="story-profile"/>
            </div>
            <div class="h-4"></div>
            <!-- Viewer -->
            ...
            <div :class="darkMode?'bg-secondary text-textSecondary':'bg-white'"
                 class="absolute z-50 left-0 w-full h-full overflow-y-scroll  rounded-xl shadow-md" @scroll="onScroll"
                 v-if="showViewer">
                ...
                <div class="mt-5">
                    <div v-for="(viewer,index) in storyDetail.viewer" :key="index"
                        ...
                        <div class="flex items-center mb-5 px-5">
                            <div class="w-12 h-12 rounded-full bg-cover bg-gray-200 mr-5 bg-center cursor-pointer z-50 cursor-pointer"
                                 :style="{backgroundImage:`url(${viewer.photo || 'poster.png'})`}"
