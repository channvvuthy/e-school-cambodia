    ...
    <div v-else>
      ...
      <div class="grid gap-6 md:grid-cols-3 2xl:grid-cols-4 px-5 mt-5" v-if="(partners && partners.length)">
        <div v-for="(partner, index) in partners" :key="index" class="rounded-xl shadow-md"
             :class="darkMode?`bg-secondary text-gray-300 ${partner.packages.length > 1?`col-span-2`:``}`:`bg-white ${partner.packages.length > 1?`col-span-2`:``}`">
          <div>
            <div class="flex items-center p-3">
              <div class="h-12 w-12 bg-cover rounded-full bg-center" :class="darkMode?`bg-byline`:`bg-gray-300`"
                   :style="{backgroundImage:`url(${partner.photo})`}"></div>
            ...
            <template v-else>
              ...
              <div class="flex px-3 items-center h-14 justify-between">
                <div class="text-sm">{{ $t('1006') }}:
                  <del class="mx-2 font-semibold">${{ partner.packages[0].price.discount }}</del>
