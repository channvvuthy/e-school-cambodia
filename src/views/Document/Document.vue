<template>
  <div>
    <div :class="darkMode?`bg-youtube text-gray-300 mt-5 px-5 `:`mt-5 px-5 `" class="h-screen pb-80 overflow-y-scroll"
         @scroll="onScroll">
      <div class="w-1/2">
        <div class="relative">
          <input type="text" class="w-full h-12 border rounded-lg pl-4 focus:outline-none shadow"
                 :placeholder="$t('1001')" :class="darkMode?`bg-secondary border-button`:``" v-model="s"
      ...
      <template v-else>
        <div class="grid md:grid-cols-3 2xl:grid-cols-4 gap-5">
          <div class="rounded-xl shadow w-full p-5" :class="darkMode?`bg-secondary`:`bg-white`"
            ...
            <div class="font-semibold text-lg mt-5" :class="darkMode?`text-gray-300`:`text-primary`"
                 :title="document.name">
    ...
    <div class="fixed bottom-0 right-0 bg-transparent h-36 w-full">
      <div class="t-shadow  w-full h-36 t-shadow p-10" :style="isHide?{marginLeft:'0px'}:{marginLeft:'350px'}"
           style="box-shadow: 0 -3px 2px rgba(50, 50, 50, 0.1);"
        ...
        <div class="flex items-center" :class="darkMode?``:`text-gray-500`">
          <div class="flex items-center cursor-pointer" @click="() =>{this.$refs.add_image.click()}">
            ...
            <input type="file" class="hidden" ref="add_image" accept="image/png, image/gif, image/jpeg"
                   @change="addImage">
          </div>
    ...
    <!-- Preview Image -->
    <div
        class="w-full h-full fixed top-0 left-0 bg-black z-50 flex items-center justify-center bg-opacity-90 p-5 overflow-y-scroll"
        v-if="showPreview">
      <div class="w-96 rounded-xl flex flex-col justify-between"
           :class="darkMode?`bg-secondary text-gray-300`:`bg-white shadow`" style="max-height:100%;">
        ...
        <div class="px-5" v-if="directories.length">
          ...
          <div>
            <select v-model="directroy" class="w-full h-10 focus:outline-none"
              ...
              <option :value="directory._id" v-for="(directory, index) in directories" :key="index">
                {{ directory.name }}
              </option>
            </select>
          </div>
    ...
    <div class="w-full h-full fixed top-0 left-0 bg-black z-50 flex items-center justify-center bg-opacity-90"
      ...
      <div class="w-96 rounded-xl flex flex-col justify-between"
        ...
        <div class="px-5">
          ...
          <select v-model="directroy" class="w-full h-10 focus:outline-none"
                  :class="darkMode?`border bg-youtube border-button text-gray-300`:`bg-white border border-gray-300`">
            <option :value="directory._id" v-for="(directory, index) in directories" :key="index">{{ directory.name }}
            </option>
          </select>
        </div>
