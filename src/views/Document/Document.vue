<template>
    <div>
        ...
        <div :class="darkMode?`bg-youtube text-gray-300 mt-5 px-5 `:`mt-5 px-5 `" class="h-screen pb-80 overflow-y-scroll" @scroll="onScroll">
            <div class="w-1/2">
                <div class="relative">
                    <input type="text" class="w-full h-12 border rounded-lg pl-3 focus:outline-none shadow" :placeholder="$t('1001')" :class="darkMode?`bg-secondary border-button`:``" v-model="s" v-on:keyup.enter="search">
        ...
        <div class="w-full h-full fixed top-0 left-0 bg-black z-50 flex items-center justify-center bg-opacity-90" v-if="showPdf">
            <div class="w-96 rounded-xl flex flex-col justify-between" :class="darkMode?`bg-secondary text-gray-300`:`bg-white shadow`">
                ...
                <div class="border-t" :class="darkMode?`border-button`:`border-gray-300`"></div>
                <div class="h-5"></div>
                <div class="px-5 flex items-center justify-center uppercase">
                    <div>
                        {{fileName}}
                    </div>
                </div>
        ...
        <!-- Viwe pdf file -->
        <div class="fixed w-full h-full left-0 top-0 bg-black bg-opacity-90 flex items-center justify-center z-50" v-if="pdfUrl">
           <div style="max-height:90%;">
               <div class="rounded-t-lg h-12 w-full bg-primary flex justify-between items-center px-3">
                ...
                <div class="overflow-y-scroll w-110 rounded-b-lg" id="fullScreen" style="max-height:36rem;"><SinglePdf :pdfUrl="pdfUrl"></SinglePdf></div>
           </div>
        </div>
        <!--  -->
        <!-- View image -->
        <div class="fixed w-full h-full bg-black bg-opacity-90 flex items-center justify-center z-50 top-0 left-0 p-10" v-if="preivewImage">
