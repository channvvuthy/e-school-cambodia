<template>
    <div class="fixed w-full h-full flex items-center justify-center top-0 left-0 bg-black z-50 bg-opacity-80">
        <div class="w-80 rounded-xl shadow" :class="darkMode?`bg-secondary text-gray-300`:`bg-white`">
            ...
            <div class="px-5 my-6">
                 <label>
                     <input type="text" v-model="directoryName" class="w-full h-10 border focus:outline-none rounded-lg px-3 text-sm" :placeholder="$t('directory_name')" ref="directory" :class="darkMode?`bg-youtube border-button`:`border-gray-300`">
                 </label>
            </div>
