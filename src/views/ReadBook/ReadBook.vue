<template>
    <div>
        <div class="overflow-y-scroll pb-40 h-screen" @scroll="onScroll">
            <div>
                <BoxFilter
                        @enableUserScroll="enableUserScroll($event)" @closeFilter="closeFilter($event)"
                        :filter_id="filter_id" @filterSearch="filterSearch($event)"/>
            </div>
            <div class="px-5 mt-5">
                <div v-if="loading">
                    <Loading/>
                ...
                <div class="grid md:grid-cols-3 2xl:grid-cols-4 gap-5" v-else>
                    <div :class="darkMode?`bg-secondary text-gray-300`:`bg-white shadow-md`"
                         class="rounded-xl p-5 flex items-center justify-center flex-col cursor-pointer"
                         v-for="(list, index) in course.list" :key="index">
                        <div class="progressBook" @click="watchDetail(list.course_id,list.percentage)">
                            <div>
                                ...
                                <div class="barOverflowBook">
                                    <div class="barBook"
                                         :style="{transform:`rotate(${45+(Math.round(list.percentage)*1.8)}deg)`}"></div>
                                </div>
                        ...
                        <div class="font-semibold flex justify-start mt-4 w-full text-left text-base"
                             :class="darkMode?`text-gray-400`:`text-primary`">
                            {{list.title}}
                        </div>
                    </div>
                </div>
