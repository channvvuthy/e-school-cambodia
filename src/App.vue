      ...
      try {
        if (localStorage.getItem("token") != null) {
          ...
          if (deeplink[3].includes("profile")) {
            ...
            let url = arg.deeplink.replace("e-school", "https");
            const params = new URL(url).searchParams;

            payload.id = params.get("id");

            if (params.get("package_id")) {
              payload.package_id = params.get("package_id");
            }
            if (params.get("order")) {
              payload.order = parseInt(params.get("order"));
            }
            this.$store.dispatch("video/getPlaylist", payload).then((res) => {
              let course = res.data.data.course;
              course.package_id = params.get("package_id");
              course.order = params.get("order");
